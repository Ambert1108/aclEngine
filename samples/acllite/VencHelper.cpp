...
AclLiteError VencHelper::Init()
{
  ...
  thread t{ &VencHelper::AsyncVencThreadEntry, this };
  asyncVencTh = std::move(t);
  //asyncVencTh.detach();
...
void VencHelper::AsyncVencThreadEntry() {
  //VencHelper* thisPtr = (VencHelper*)arg;
  DvppVenc venc(vencInfo_);

  AclLiteError ret = venc.Init();
  if (ret != ACLLITE_OK) {
    SetStatus(STATUS_VENC_ERROR);
    ACLLITE_LOG_ERROR("Dvpp venc init acl resource failed, error %d", ret);
    return;
  }

  SetStatus(STATUS_VENC_WORK);
  while (GetStatus() == STATUS_VENC_WORK) {
    shared_ptr<ImageData> image = GetEncodeImage();
    if (image == nullptr) {
  ...
  ACLLITE_LOG_INFO("VencHelper DestroyResource ---- 2 ----");
}
...
void VencHelper::DestroyResource() {
  if(vencProc_) vencProc_->Finish();
  SetStatus(STATUS_VENC_FINISH);
  ACLLITE_LOG_INFO("VencHelper DestroyResource ---- 1 ----");
  if (asyncVencTh.joinable()) asyncVencTh.join();
  ACLLITE_LOG_INFO("VencHelper DestroyResource ---- finish ----");
}
...
void DvppVenc::DestroyResource()
{
  ...
  ACLLITE_LOG_INFO("DvppVenc DestroyResource finish");
}