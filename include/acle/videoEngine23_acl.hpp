...
// @birth: [Ambert@2024.8.21]
// @version: V0.0.1
// @revision: [Ambert@2024.8.21]
...
#define MTU 1300
  ...
  public:
    ...
    void setCodecContext(AVCodecContext* codeCtx) {
      initCodectx = false;
      codec_ctx = codeCtx;
    }

    void enableHwDevice(AVBufferRef* hwDeviceContext, AVPixelFormat fmt) {
    ...
    int getFrame(const AVPacket* packet, AVFrame* frame) {
      ...
      if (ret < 0) {
        E_LOG("Error sending a packet for decoding,ret={}", ret);
        return 2;
      }
  ...
  public:
    ...
    AVCodecContext* getContext() {
      if (encodec_ctx) return encodec_ctx;
      if (useGPU)
        encodec = avcodec_find_encoder_by_name("h264_ascend");
      else
    ...
    void rateControlPreset(const int& maxRate) {
      ...
      av_dict_set_int(&dict, "zerolatency", 1, 0);
      W_LOG("User is using encoder preset.");
      while ((pEntry = av_dict_get(dict, "", pEntry, AV_DICT_IGNORE_SUFFIX))) {
        I_LOG("metadata : {}={}\n", pEntry->key, pEntry->value);
      }
      auto ctx = getContext();
      ctx->rc_max_rate = maxRate;
      ctx->bit_rate = ctx->rc_max_rate - 300000;
      int quantify = ctx->bit_rate / 1000000;
      ctx->rc_buffer_size = ctx->bit_rate * 0.5 * (1 - (quantify * 0.2));
      ctx = nullptr;
    }
    ...
    void open(AVDictionary* options = nullptr) {
      ...
      if (useGPU) {//GPU模式                 
        ...
        if (options == nullptr) {
          ...
          if (avcodec_open2(encodec_ctx, encodec, &dict) < 0) {
          ...
          while ((pEntry = av_dict_get(options, "", pEntry, AV_DICT_IGNORE_SUFFIX))) {
            I_LOG("metadata : {}={}\n", pEntry->key, pEntry->value);
          }
    ...
    void close() {
      if (encodec_ctx) {
        avcodec_flush_buffers(encodec_ctx);
  ...
  public:
    ...
    ~Demux23() { I_LOG("~Demux23"); close(); }

    void init(const char* filename = nullptr, AVBufferRef* hw_device_ctx = nullptr) {
      AVCodec* codec = nullptr;
      if (filename == nullptr) {
        codec = avcodec_find_decoder_by_name("h264");
        if (!codec) {
    ...
    void resetFmatCtx(const char* filename) {
      if (fmt_ctx) {
    ...
    int getPacket(AVPacket* inPacket) {
      int err;
